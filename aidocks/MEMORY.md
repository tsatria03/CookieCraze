...
- [Endless quests plan](project_endless_quests_plan.md) — approved, ready to build: infinitely-escalating quests (threshold = base x growth^count) that get harder each time you complete them and prestige, mirroring endless prestige. New per-quest config fields `growth`/`infinite`/`difficulty_step`; tracked in a `questEndlessCounts` dict; each completion permanently adds +1 to the prestige-points multiplier; the rank ladder becomes one endless rank quest (Section 5).
