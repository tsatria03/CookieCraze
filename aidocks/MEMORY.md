...
- [Endless prestige plan](project_endless_prestige_plan.md) — SHIPPED 6.7. Repeatable/compounding prestige upgrades: 4 flat categories (Standard/Endless × Passive/Head Start), base 5 × mult 1.1, per-item counts, unlock after clearing Standard at prestige 10. Kept as design record.
- [Cookie XP rework plan](project_cookie_xp_rework_plan.md) — approved plan to rank on a dedicated cumulative `cookieXP` (SimpleFighter-style) so selling no longer costs rank progress; splits the redundant sliders into a rank modifier (threshold) + experience modifier (XP per bake); fully specified (4 build sections; XP = 1 cookie × modifier, safe_capped; migration seeds cookieXP from the current rank's threshold). Build after endless prestige.
