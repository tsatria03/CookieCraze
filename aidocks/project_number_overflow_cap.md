...
**Shipped 6.3** (version.txt bumped, changelog block added). Implementation as below; kept the `1e308` ceiling and `double` types. If `Inf` still appears somewhere, the miss is an add/display local that skips both a `floor(safe_cap(...))` normalizer and the loop clamp — search for a value shown by `format_number`/`convert_to_currency` whose source isn't wrapped. (`format_number` returns the literal `"Inf"` only for values strictly `> 1e308`, so a safe-capped `1e308` displays fine as `1e+308`.)

**Watch aggregates that sum already-capped values (6.3 follow-up).** `safe_cap` on each *item* cost isn't enough when several are summed: a bundle price loops `totalCost += calc_total_upgrade_cost(...)` over its entries, and N capped values can sum past `1e308` again (the rank-500 ultimate bundle showed "Costs Inf"). Fixed by routing all six duplicated bundle-cost loops in `menu.nvgt` through one helper, `calc_bundle_total(bundle, storeItems)`, which caps progressively (`total = safe_cap(total + ...)`). Rule of thumb: any running total of capped values needs its own `safe_cap`, per addition. The same audit also capped the "buy all affordable" summary accumulators in `menu.nvgt` (`totalSpent`, `autoGained`/`manulGained`/`speedGained`, and the `*PercentGained` totals) in both the singles and bundle shops — they summed capped per-item values and were shown via `format_number`/`convert_to_currency`. The ticket shop was already safe (per-ticket cost is `safe_cap`ped in `get_ticket_current_cost`, and the `cost * qty` purchase total is guarded by an affordability check so an overflow is rejected before display).
