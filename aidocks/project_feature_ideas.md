...
- **Roulette** ⭐⭐ — reuses the bet selector + spin/reveal (slots) + a payout table keyed by bet type. **The standout new mechanic: MULTIPLE simultaneous bets on a single spin** (some on red, some on a single number, some on even, etc.). *Every current minigame is one wager per round*, so multi-betting is roulette's real justification, not a reskin. Also brings bet-type-driven odds (single number pays big, red/black/even/odd pays small) — a discrete cousin of dice's target-setting.
  ...
  - **Auto/manual toggle (dev-decided 2026-08-24):** roulette gets an auto-mode bool (e.g. `roulettemode`, default true) + a game-settings checkbox like every other minigame, modeled on **dice** (`minigames.nvgt`, `dicemode`). **Auto** plays the full decelerating tick-loop spin animation, then speaks the result. **Manual** shows a `dlg_buffer("The wheel is spinning. Press enter or space to see where it lands.")` that skips the animation and jumps straight to `land_sound` + the result dialog — genuinely useful since the full spin is ~7s and repeat players may want to skip it. Persist in the game-settings preffs + `reset_game_settings()` default, the standard minigame settings pattern ([[project_minigame_build_guide]] §3/§6).
